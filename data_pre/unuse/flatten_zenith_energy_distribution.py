...
parser.add_argument("-i", "--input_files",default='NuMu_genie_149999_00????_level6.zst_cleanedpulses_transformed_IC19.hdf5',
...
parser.add_argument("-d", "--path",type=str,default='/mnt/scratch/yushiqi2/training_files/L55Sim1000/',
                    dest="path", help="path to input files")
parser.add_argument("-od", "--out_path",type=str,default='/mnt/scratch/yushiqi2/training_files/L55Sim1000/',
...
parser.add_argument("-v", "--variable", type=str,default="energy",
                    dest="variable", help="flattening which var")

parser.add_argument("-b", "--bin_size",type=float,default=1,
                    dest="bin_size", help="Size of zenith bins in ")
parser.add_argument("--max_per_bin",type=int,default=20000,
                    dest="max_evt_per_bin",help="Max number of zenith events per bin")
parser.add_argument("--var_max",type=float,default=300.0,
                    dest="var_max",help="Cut anything greater than this energy (in GeV)")
parser.add_argument("--var_min",type=float,default=0.0,
                    dest="var_min",help="Cut anything less than this energy (in GeV)")
...
if transformed:
    ...
    if args.variable == "energy":
      var_max=var_max/efactor
      var_min=var_min/efactor
      bin_size=bin_size/efactor
else:
    azimuth_index = 2
    track_index = 7
    tfactor = 1.0
...
print("Saving PEGLEG info: %s \nBin size: %f GeV \nMax Count Per Bin: %i events \nEnergy Range: %f - %f \nStarting cut: %s \nEnding cut: %s \nShuffling: %s \nKeeping event types: %s"%(use_old_reco,bin_size,max_events_per_bin,var_min,var_max,start_cut,end_cut,shuffle,cut_name)) 
if transformed:
...
bins = int((var_max-var_min)/float(bin_size))
if var_max%bin_size !=0:
    bins +=1 #Put remainder into additional bin
count_variable = np.zeros((bins))
...
for a_file in event_file_names:
    ...
    if args.variable == "zenith":
      variable=file_labels[:,1]
    elif args.variable == "energy":
      variable=file_labels[:,0]
    else:
      print("wrong var")
      exit
    keep_index = [False]*len(variable)
    print("Total events this file: %i"%len(variable))
    ...
    for index,var in enumerate(variable):
        ...
        var_bin = int((var-var_min)/float(bin_size))
        if count_variable[var_bin] < max_per_bin:
            keep_index[index] = True
            count_variable[var_bin] += 1
    ...
    print("Total events saved: %i, Saved this file: %i, Finsihed file: %s"%(sum(count_variable),sum(keep_index),a_file))
    ...
    if np.all(count_variable >= max_per_bin):
...
if count_out_bounds > 0:
    print("Got rid of %i events out of the %s bounds [%f, %f]"%(count_out_bounds, variable, var_min, var_max))

# CHECKS
if args.variable == "zenith":
  variable=full_labels[:,1]

elif args.variable == "energy":
  variable=full_labels[:,0]
else:
  print("has to be zenith or energy")
  quit()
over_max = variable > var_max
under_min = variable < var_min
assert sum(over_max)==0, "Have events greater than max in final sample"
assert sum(under_min)==0, "Have events less than min in final sample"
if cut_name == "CC":
...
#    fraction_test=0.1,fraction_validate=0.2)
    fraction_test=0.0,fraction_validate=0.2)
    ...
    for sep_file in range(0,num_outputs):
        ...
        if num_outputs > 1:
            ...
            filenum = ""
        output_name = out_path + output + "_"+args.variable+"_%i"%var_min + "to%i_"%var_max + "%s_"%cut_name + "%s_%s_"%(start_cut,end_cut) + "flat_%sbins_%sevtperbin"%(bins,max_events_per_bin) + filenum + ".hdf5"
    ...
    for sep_file in range(0,num_outputs):
        ...
        if num_outputs > 1:
            ...
            filenum = ""
        output_name = out_path + output + "lt%03d_"%var_max + "%s_"%cut_name + "%s_%s_"%(start_cut,end_cut) + "flat_%sbins_%sevtperbin"%(bins,max_events_per_bin) + filenum + ".hdf5"
