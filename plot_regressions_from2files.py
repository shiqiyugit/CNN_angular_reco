...
parser.add_argument("-i", "--input2",type=str,dest="input2",
                    default="/mnt/home/micall12/LowEnergyNeuralNetwork/output_plots/MuonClassification_level4_9999set_130000_10k-11k/prediction_values_retro_120000_all_140000_all_160000_all_130000_all.hdf5", help="path and name of the input file, retro")
parser.add_argument("--input1",type=str,dest="input1",
                    default="/mnt/home/micall12/LowEnergyNeuralNetwork/output_plots/MuonClassification_level4_9999set_130000_10k-11k/prediction_values_BDT.hdf5", help="path and name of the input file")
parser.add_argument("-o", "--outdir",type=str,default='./nufact_plots/',
...
if i3:
    ...
    variable_list = ["energy", "prob_track", "zenith", "vertex_x", "vertex_y", "vertex_z", "prob_muon", "nDOM", "end_x", "end_y", "end_z", "prob_nu"] #syu
...
reco1['prob_nu'] = np.array(predict1[:,-1])#syu BDT
...
#mask1['ProbMu'] = reco1['prob_nu'] < cut1['mu'] #<= cut1['mu']
mask1['ProbNu'] = reco1['prob_nu']>=cut1['nu']
mask1['Reco'] = np.logical_and(mask1['ProbNu'], np.logical_and(mask1['Zenith'], np.logical_and(mask1['Energy'], mask1['Vertex'])))
mask1['DOM'] = reco1['nDOMs'] >= cut1['nDOM']
mask1['RecoNoEn'] = np.logical_and(mask1['ProbNu'], np.logical_and(mask1['Zenith'], mask1['Vertex']))
mask1['RecoNoZenith'] = np.logical_and(mask1['ProbNu'], np.logical_and(mask1['Energy'], mask1['Vertex']))
mask1['RecoNoZ'] = np.logical_and(mask1['ProbNu'], np.logical_and(mask1['Zenith'], np.logical_and(mask1['Energy'], mask1['R'])))
mask1['RecoNoR'] = np.logical_and(mask1['ProbNu'], np.logical_and(mask1['Zenith'], np.logical_and(mask1['Energy'], mask1['Z'])))
...
true1['Z'] = np.logical_and(true1['z'] > cut1['zmin'], true1['z'] < cut1['zmax'])
true1['R'] = true1['r']< cut1['r']
true1['Vertex'] = np.logical_and(true1['Z'], true1['R'])
true1['isNuMuCC'] = true1['isNu'] & true1['isNuMu']
...
if weights1 is not None:
    if sum(true1['isNuMu']) > 1:
        ...
        weights1[true1['isNuMuCC']] = weights1[true1['isNuMuCC']]/numu_files1
...
if info1 is not None:
    more_info1['prob_nu'] = info1[:,-1] #syu BDT1]
...
if info1 is not None:
    ...
    mask1['oscNext_Nu'] = more_info1['prob_nu'] > 0.8 #syu
...
#    mask1['AnalysisNoVertex'] = np.logical_and(np.logical_and(mask1['CoinHits'],mask1['Noise']),mask1['RecoNoVertex'])
...
if input_file2 is not None:
    ...
    true2['Z'] = np.logical_and(true2['z'] > cut2['zmin'], true2['z'] < cut2['zmax'])
    true2['R'] = true2['r']< cut2['r']
    true2['Vertex'] = np.logical_and(true2['Z'], true2['R'])
...
#        reco2['prob_mu'] = np.array(predict2[:,-1])
        reco2['nDOMs'] = np.array(predict2[:,7])

        #RECO masks
        mask2['ProbNu'] = reco2['prob_mu'] <= cut2['mu']
    ...
    true2['isNuMuCC'] = true2['isCC'] & true2['isNuMu']
    ...
    if weights2 is not None:
        if sum(true2['isNuMu']) > 1:
            ...
            weights2[true2['isNuMuCC']] = weights2[true2['isNuMuCC']]/numu_files2
...
#Additional info
    ...
    if info2 is not None:
        ...
        mask2['oscNext_Nu'] = more_info2['prob_nu'] > 0.8 #syu
    ...
    mask2['Reco'] = np.logical_and(mask2['ProbNu'], np.logical_and(mask2['Zenith'], np.logical_and(mask2['Energy'], mask2['Vertex'])))
    mask2['RecoNoEn'] = np.logical_and(mask2['ProbNu'], np.logical_and(mask2['Zenith'], mask2['Vertex']))
    mask2['RecoNoZenith'] = np.logical_and(mask2['ProbNu'], np.logical_and(mask2['Energy'], mask2['Vertex']))
    mask2['RecoNoZ'] = np.logical_and(mask2['ProbNu'], np.logical_and(mask2['Zenith'], np.logical_and(mask2['Energy'], mask2['R'])))
    mask2['RecoNoR'] = np.logical_and(mask2['ProbNu'], np.logical_and(mask2['Zenith'], np.logical_and(mask2['Energy'], mask2['Z'])))
...
var_names = ["Neutrino Energy", "$\cos(\theta_{zenith})", "Z Position", "Radius", "X End", "Y End", "Z End", "R End", "X Position", "Y Postition"]
...
############## CHANGE THESE LINES ##############
variable_index_list = [2, 3] #[0] #[1,2,3] #chose variable from list above
check_index_list = [0, 1] #[0,1,2,3] #corresponds to flavor/select index
...
if make_muon:
#NEED BINARY PROBMU
    percent_save = my_confusion_matrix(true1['isNu'], mask1['ProbNu'], weights1,
                    mask=mask1['Analysis'],title="%s Muon Cut"%name1,
                    save=save,save_folder_name=save_folder_name)

    if input_file2 is not None:
        percent_save = my_confusion_matrix(true2['isNu'], mask2['ProbNu'], 
                    weights2,
...
#NEED BINARY PROB TRACK
    ...
    percent_save = my_confusion_matrix(true1['isTrack'], reco1['prob_track'],
                    weights1,label0="Cascade",label1="Track",
                    mask=mask1_here,title="%s PID Cut"%name1,
                    save=save,save_folder_name=save_folder_name)
    if input_file2 is not None:
        mask2_here = np.logical_and(mask2['Analysis'], true2['isNu'])
        percent_save = my_confusion_matrix(true2['isTrack'], reco2['prob_track'],
...
#    reco1_binary = np.logical_or(reco1[variable_name][minus_var_mask1] > cut_min, reco1[variable_name][minus_var_mask1] < cut_max)
    ...
    mask1_here = true1['isNuMuCC']
    percent_save = my_confusion_matrix(true1['Vertex'], mask1['Vertex'],
                    weights1,label0="Uncountained", label1="Conatined",
                    mask=mask1_here,title="%s NuMu CC"%name1,
                    save=save,save_folder_name=save_folder_name)

    if input_file2 is not None:
        mask2_here = true2['isNuMuCC'] #np.logical_and(mask2['Analysis'], true2['isNu'])
        percent_save = my_confusion_matrix(true2['Vertex'], mask2['Vertex'], 
                    weights2,label0="Uncountained", label1="Conatined",
                    mask=mask2_here,title="%s NuMu CC"%name2,
                    save=save,save_folder_name=save_folder_name, purity=True, color="Reds")
    ...
    for check_set in check_index_list:
        ...
        if make_bin_slice:
            ...
            plot_bin_slices(true1_value_fullAnalysis, reco1_value_fullAnalysis, 
                        ...
                        xvariable="%s Neutrino Energy"%energy_type,xunits="(GeV)",
