...
def plot_single_resolution(truth,nn_reco,weights=None, \
    ...
    if use_fraction:
        ...
        xlabel = "Reco. - True / True"#r'$\frac{reconstruction - truth}{truth}$'
    else:
        nn_resolution = nn_reco - truth
        if use_old_reco:
            old_reco_resolution = old_reco - truth2
        title = ""#%s Resolution"%variable
        xlabel = "Reco. - True"#reconstruction - truth %s"%units
    if epochs:
        title += " at %i Epochs"%epochs
    if flavor == "NuMu" or flavor == "numu":
        title += r' for $\nu_\mu$ ' 
...
def plot_bin_slices(truth, nn_reco, energy_truth=None, weights=None,\
    ...
    if vs_predict:
        ...
        plt.xlabel("%s Neutrino %s %s"%(variable_type,variable,xunits),fontsize=20)
    if use_fraction:
        plt.ylabel(r'Reco. - True / True %s'%variable,fontsize=20) #syu)r'Fractional Resolution: $\frac{reconstruction - truth}{truth}$',fontsize=20)
    else:
         plt.ylabel(r'Reco. - True %s'%variable,fontsize=20) #syu Resolution: \n reconstruction - truth %s"%units,fontsize=20)
    if xlog:
        plt.xscale('log')
