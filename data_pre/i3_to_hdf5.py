...
parser.add_argument("--check_unique", default=False,action='store_true',
                        dest='check_unique',help="check for uniqueness of event")
parser.add_argument("-ic", "--icring",type=str,default="all, none, none, none",
...
if args.check_unique:
  print("will check uniqueness of events based on zenith values")
...
def read_files(filename_list, use_old_reco, check_filters, true_name, reco_type, check_unique=False):
    """
    ...
    for event_file_name in filename_list:
        ...
        for frame in event_file:
            if frame.Stop == icetray.I3Frame.Physics:
                ...
                #else:
                #always save the correct weights
                if (neutrino_type==14):
                  weights["weight_no_osc"]=weights["NormalizedOneWeight"]*weights["flux_mu"]
                elif (neutrino_type == 12):
                  weights["weight_no_osc"]=weights["NormalizedOneWeight"]*weights["flux_e"]
                elif (neutrino_type == 16): 
                  weights["weight_no_osc"]=weights["NormalizedOneWeight"]*weights["flux_tau"]

                output_weights.append( np.array([ float(header.run_id), float(header.sub_run_id), float(header.event_id), float(weights["NEvents"]), float(weights["OneWeight"]), float(weights["GENIEWeight"]),float(weights["PowerLawIndex"]), float(weights["gen_ratio"]), float(weights["flux_mu"]), float(weights["weight"]) , float(weights["weight_no_osc"]) ]) )
    ...
    if check_unique:
      print("checking uniqueniess...")
      zenith=output_labels[:,1]
      ...
      output_features_DC=output_features_DC[keep]
      output_features_IC=output_features_IC[keep]
      output_labels=output_labels[keep]
      output_initial_stats=output_initial_stats[keep]
      output_num_pulses_per_dom=output_num_pulses_per_dom[keep]
      output_trigger_times=output_trigger_times[keep]
      output_weights=output_weights[keep]
...
features_DC, features_IC, labels, reco_labels, initial_stats, num_pulses_per_dom, trigger_times, weights, ICstrings = read_files(event_file_names, use_old_reco, check_filters, true_name, reco_type, check_unique=args.check_unique)
