...
parser.add_argument("-i", "--input",type=str,default="/mnt/research/IceCube/jmicallef/simulation/level6/149999/NuMu_genie_149999_??????_level6.zst",
#/mnt/research/IceCube/yushiqi2/Files/Flat_Zenith/level6_oldgcd/1st/NuMu_genie_149999_00????_level6.zst",
                    dest="input_file", help="path and name of the input file")
parser.add_argument("-n", "--name",default=None,
                    dest="output_name",help="name for output file (no path)")
parser.add_argument("-o", "--outdir",type=str,default='./',
#/mnt/research/IceCube/yushiqi2/Files/Flat_Zenith/level6_hdf5/',
                    dest="output_dir", help="path of ouput file")

parser.add_argument("-p", "--use_pulses", type=bool,default=False,
                    dest="use_pulses", help="use full pulses up to 100?")
parser.add_argument("-ic", "--icring",type=str,default="all, all, single, none",
                    dest="IC_ring", help="status of ic rings (19,37,61,86) : all, single or none")
...
def get_observable_features(frame,low_window=-500,high_window=4000):
    ...
    IC37 = [9, 10, 11, 12, 16, 20, 24, 29, 33, 39, 43, 48, 53, 57, 62, 63, 64, 65]
    IC61 = [2, 3, 4, 5, 6, 8, 13, 15, 21, 23, 30, 32, 40, 42, 49, 52, 58, 61, 66, 69, 70, 71, 72, 73]
    IC86 = [1, 7, 14, 22, 31, 41, 50, 51, 59, 60, 67, 68, 74, 75, 76, 77, 78]
    ...
    rings[0] = [36, 26, 27, 35, 37, 45, 46, 17, 18, 19, 25, 28, 34, 38, 44, 47, 54, 55, 56] #--> IC19
    rings[1] = [9, 10, 11, 12, 16, 20, 24, 29, 33, 39, 43, 48, 53, 57, 62, 63, 64, 65] #-->IC37
    rings[2] = [2,3,4,5,6,8,13,15,21,23,30,32,40,42,49,52,58,61,66,69,70,71,72,73] #-->IC61
    rings[3] = [1, 7, 14, 22, 31, 41, 50, 51, 59, 60, 67, 68, 74, 75, 76, 77, 78] #-->IC86

#    include = args.IC_ring #["all", "none", "none", "none"]
    include=args.IC_ring.split(',')
    DC_strings = [79, 80, 81, 82, 83, 84, 85, 86]

    IC_near_DC_strings = []
    for i in range(len(include)):
        if include[i] == "all":
            IC_near_DC_strings += rings[i]
        elif include[i] == "single":
            IC_near_DC_strings.append(100+i)
    ICstrings = len(IC_near_DC_strings)
    ...
    array_DC = np.zeros([len(DC_strings),60,5])
    array_IC_near_DC = np.zeros([len(IC_near_DC_strings),60,5])

    if args.use_pulses:
      print("use pulses")
      array_DC = np.zeros([len(DC_strings),60,100,2]) # [string, dom_index, charge & time summary]
      array_IC_near_DC = np.zeros([len(IC_near_DC_strings),60,100,2]) # [string, dom_index, charge & time summary]
    ...
    print("trigger_time: ", shift_time_by)

    #Start by making all times negative shift time (to distinguish null from 0)
    array_DC[...,1:] = -20000
    array_IC_near_DC[...,1:] = -20000
 
    combo_chargelists = [[[] for x in range(60)] for x in range(len(include))]
    combo_timelist = [[[] for x in range(60)] for x in range(len(include))]
    ...
    if clean_pulses_8_or_more == True:
        for omkey, pulselist in ice_pulses:
            ...
            for i in range(len(include)):
              if string_val in rings[i] and include[i] == "single":
                  string_val = 100+i
                  timelist = combo_timelist[i][dom_index]
                  chargelist = combo_chargelists[i][dom_index]
            ...
            if DC_flag == True:
              if args.use_pulses:
                array_DC[string_index,dom_index,0:charge_array.shape[0],0] = charge_array
                array_DC[string_index,dom_index,0:charge_array.shape[0],1] = time_array
              else:
            ...
            if IC_near_DC_flag == True:
              if args.use_pulses:
                array_IC_near_DC[string_index,dom_index,0:charge_array.shape[0],0] = charge_array
                array_IC_near_DC[string_index,dom_index,0:charge_array.shape[0],1] = time_array
              else:
...
def apply_transform(features_DC, features_IC, output_labels=None, energy_factor=100., track_factor=200.,transform="MaxAbs",transform_output=True):
    ...
    if transform_output and len(output_labels)!=0:
        ...
        output_labels[:,1] = output_labels[:,1] #zenith
...
def read_files(filename_list, use_old_reco, check_filters, true_name, reco_type):
    ...
    # calculate sparsity
    sparsity_DC = 1.0 - (np.count_nonzero(output_features_DC) / float(output_features_DC.size))
    sparsity_IC = 1.0 - (np.count_nonzero(output_features_IC) / float(output_features_IC.size))
    print("DC sparsity:", sparsity_DC, "IC sparsity:",sparsity_IC)
        
    print("read file output: ", output_labels)
