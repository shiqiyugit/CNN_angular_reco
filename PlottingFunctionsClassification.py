...
def my_confusion_matrix(binary_truth, binary_class, weights, mask=None, color="Blues",
                     ...
                     title="CNN Muon Cut",save=True,save_folder_name=None,notebook=False, purity=False):
    ...
    #fig.colorbar(im, vmin=minval, vmax=maxval) #, orientation='vertical')#, vmin=minval, vmax=maxval)
    plt.yticks(ticks=[0.25,0.75],labels=["%s"%label0, "%s"%label1],fontsize=20, rotation=90, va='center')
    plt.xticks(ticks=[0.25,0.75],labels=["%s"%label1, "%s"%label0],fontsize=20, ha='center')
    ax.set_ylabel("%s"%ylabel,fontsize=25,rotation=90)
    ax.set_xlabel("%s"%xlabel,fontsize=25)
    if purity:
        ax.set_title("Purity Matrix", fontsize=30)
    else:
        ax.set_title("Efficiency Matrix", fontsize=30)
    ...
    reco_one = binary_class == 1
    reco_zero = binary_class == 0
    mask_reco_one = np.logical_and(reco_one,mask)
    mask_reco_zero = np.logical_and(reco_zero,mask)
    if purity:
        mask_one = mask_reco_one
        mask_zero = mask_reco_zero
    else:
        mask_one = mask_true_one
        mask_zero = mask_true_zero
...
#        xbins = column
    for i in range(len(xbins)-1):
        for j in range(len(ybins)-1):
            c="k"
            if j == 0:
                total = sum(weights[mask_one])
            if j == 1:
                total = sum(weights[mask_zero])
            events = hist.T[i,j]
